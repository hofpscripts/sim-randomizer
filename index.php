...
?>
<h1 class="text-center page-header">SIM Randomizer</h1>
...
if($showe == TRUE){
  ...
  if(strpos($className,'[')){
        $className2 = trim(strstr($className, '[',TRUE));
       // $num = getStringBetween($className,'[',']');
preg_match('/\[(?P<num>\d+)\]/', $className, $matches);
$num = $matches['num'];
       // echo $num;
        include 'inc/multi.php';
        //rinclude 'inc/newrand.php';
        }else{
        $className = $className;
...
<div class="col-sm-6">
<h4>Multi-Ran Classes</h4>
<p><em>This is handy for eventing, jumping, dressage, and other multi-test/round classes. Class is ran the number of times specified and each test/round is named appropriatly.<b>You must choose a discipline if it's available.</b></em></p>
<p>Class Name [#]<br />
Horse<br />
Horse<br />
<br />
 ...
 <div class="form-group">
     ...
     <input type="text" class="form-control input-sm" name="showName"/>
      ...
      <input type="text" class="form-control input-sm" name="host"/>
       ...
       <div class="form-group">
      <label for="name" class="col-lg-2 control-label">Discipline</label>
      <div class="col-lg-10">
      <select class="form-control input-sm" name="disc">
      <option></option>
<?php
if ($file = @fopen('inc/disc.txt', 'r')) {
  while(($line = fgets($file)) !== false) {
  $data = explode(",", $line);
		echo "<option value=" .trim($data[1]). ">" .$data[0]. "</option>";
	}
	fclose($file);
}
?>
 ...
 <div class="form-group">
     ...
     <div class="form-group">
        <div class="col-lg-10 col-lg-offset-2 checkbox">
          <label>
            <input type="checkbox" name="noEntries" value="1"> Display # of Entries</label>
          </div></div>
          <div class="offset-3 form-group">
        <div class="col-lg-10 col-lg-offset-2 checkbox">
          <label>
            <input type="checkbox" name="tenonly" value="1"> Show Top 10 Placings Only</label>
          </div> </div>
    <div class="form-group">
